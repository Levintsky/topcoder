...
class Solution(object):
    def findLadders(self, beginWord, endWord, wordList):
        ...
        if not wordList:
            return []
        wordn = len(wordList[0])
        wordList = set(wordList + [beginWord])
        for word1 in wordList:
            for i in range(wordn):
                for j in range(26):
                    c = chr(ord('a') + j)
                    word2 = word1[:i] + c + word1[i+1:]
                    if word1 != word2 and word2 in wordList: 
                        if word1 not in memo:
                            memo[word1] = []
                        if word2 not in memo:
                            memo[word2] = []
                        memo[word1].append(word2)
                        memo[word2].append(word1)
        ...
        while len(q) > 0:
            ...
            while q:
                w = q.popleft()
                if w in memo: 
                    for next_word in memo[w]: # see this word before
                        if next_word not in visited:
                            if next_word not in res:
                                res[next_word] = set()
                            res[next_word].add(w)
                            tmp_visit.add(next_word)
                            q2.append(next_word)
            visited = visited | tmp_visit
            # print('q2', q2)
        ...
        # post-process
        result = []
        def back_track(w, end_w, tmp_result):
            if w == end_w:
                result.append(tmp_result[::-1])
                return
            if w in res:
                for tmpw in res[w]:
                    back_track(tmpw, end_w, tmp_result+[tmpw])

        back_track(endWord, beginWord, [endWord])
        return result

    def solve2(self, start, end, dic):
        dic = set(dic)
        dic.add(end)
        level = {start}
...
if __name__ == "__main__":
    ...
    print(a.findLadders("hit", "cog", ["hot","dot","dog","lot","log"]))
    ...
    print(a.findLadders(w1, w2, arr))
    print(a.solve2(w1, w2, arr))
