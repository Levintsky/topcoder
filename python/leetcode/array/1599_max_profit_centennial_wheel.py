...
ou are the operator of a Centennial Wheel that has four gondolas, and each gondola has room for up to four people. You have the ability to rotate the gondolas counterclockwise, which costs you runningCost dollars.
    ...
    def minOperationsMaxProfit(self, customers, boardingCost, runningCost):
        ...
        for i, item in enumerate(customers, start=1):
            ...
            if res > best:
                best = res
                best_idx = i
            print(i, item, best, best_idx)

        while waiting > 0:
            i += 1
            board = min(waiting, 4)
            res += board * boardingCost - runningCost
            waiting -= board
            if res > best:
               best = res
               best_idx = i
...
if __name__ == "__main__":
    ...
    print(a.minOperationsMaxProfit([3, 4, 0, 5, 1], 1, 92))
