...
a building located there. We are allowed to increase the height of any number of 
...
bottom, left, and right, must be the same as the skyline of the original grid. A 
city's skyline is the outer contour of the rectangles formed by all the buildings 
...
class Solution(object):
    def maxIncreaseKeepingSkyline(self, grid):
        ...
        max_m = [max(item) for item in grid]
        max_n = []
        for i in range(n):
            tmp = [grid[j][i] for j in range(m)]
            max_n.append(max(tmp))
        
        res = 0
        for i in range(m):
            for j in range(n):
                best = min(max_m[i], max_n[j])
                res += best - grid[i][j]
        return res