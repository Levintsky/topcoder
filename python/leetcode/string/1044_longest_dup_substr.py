...
class SuffixArrayInducedSort:
    ...
    def _build_buckets(self, s):
        buckets = sorted(Counter(s).iteritems())
        indices = {c: i for i, (c, _) in enumerate(buckets)}
        self._string = map(indices.__getitem__, s)
        self._n = len(self._string)
        start = [1]
        for _, size in buckets: start.append(start[-1] + size)
    ...
    def _lms_equal(self, i, j):
        ...
        if i == j: return True
        if i == self._n or j == self._n: return False
        k = 0
        while True:
            ilms = self._is_lms(i+k)
            jlms = self._is_lms(j+k)
            if k > 0 and ilms and jlms: return True
            if ilms != jlms or self._string[i+k] != self._string[j+k]: return False
            k += 1
    ...
    def _guess_suf(self):
        ...
        for i, c in enumerate(self._string):
            if not self._is_lms(i): continue
            suf[end[c]] = i
            end[c] -= 1
        suf[0] = self._n
    ...
    def _induce_sort(self, suf):
        ...
        for i in xrange(self._n+1):
            j = suf[i] - 1
            if j < 0 or self._s_type[j]: continue
            c = self._string[j]
            suf[start[c]] = j
            start[c] += 1
        end = list(self._bucket_end)
        for i in xrange(self._n, -1, -1):
            j = suf[i] - 1
            if j < 0 or not self._s_type[j]: continue
            c = self._string[j]
            suf[end[c]] = j
            end[c] -= 1
        return suf
    ...
    def _build_lms_suf(self):
        ...
        for i in xrange(1, self._n+1):
            curr = suf[i]
            if not self._is_lms(curr): continue
            lms[curr] = lms[last]
            if not self._lms_equal(last, curr): lms[curr] += 1
            last = curr
    ...
    def _compute_lcp(self):
        ...
        idx = [0] * (n + 1)
        for i, v in enumerate(self.suf): idx[v] = i
        self.lcp = [0] * (n + 1)
        k = 0
        for i in xrange(n + 1):
            if idx[i] == n:
