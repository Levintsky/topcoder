...
f2_content ... fn_content, respectively) in directory root/d1/d2/.../dm. Note that n >= 1 
and m >= 0. If m = 0, it means the directory is just the root directory.

The output is a list of group of duplicate file paths. For each group, it contains all the 
file paths of the files that have the same content. A file path is a string that has 
...
class Solution(object):
  def findDuplicate(self, paths):
    ...
    for item in paths:
      ...
      path = item_list[0]
      for tmpf in item_list[1:]:
        i = 0
        while tmpf[i] != '(': i += 1
        fname = tmpf[:i]
        content = tmpf[i+1:-1]
        if content not in content_2_file:
          content_2_file[content] = [path + '/' + tmpf[:i]]
        else:
          content_2_file[content].append(path + '/' + tmpf[:i])
    print content_2_file
    result = []
    for k in content_2_file.keys():
      if len(content_2_file[k]) > 1: result.append(content_2_file[k])
    return result
