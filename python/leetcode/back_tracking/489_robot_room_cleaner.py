...
The robot cleaner with 4 given APIs can move forward, turn left or 
turn right. Each turn it made is 90 degrees.

When it tries to move into a blocked cell, its bumper sensor 
detects the obstacle and it stays on the current cell.

Design an algorithm to clean the entire room using only the 4 
given APIs shown below.

interface Robot {
  // returns true if next cell is open and robot 
  moves into the cell.
  // returns false if next cell is obstacle and 
  robot stays on the current cell.
  boolean move();
...
class Solution(object):
    def cleanRoom(self, robot):
        ...
        :rtype: None
        """
        # image current position is (0, 0), facing up
        # logic: dfs + backtrack
        # dfs():
        #   at a cell (i, j) do
        #   clean()
        ...
        #   for 4 directions:
        #   move()
        #   if move successfully, to (i', j'), dfs(i', j')
        #     back-track, undo the move, and face the original direction
        #     robot.turnRight();
        #     robot.turnRight();
        #     robot.move();
        #     robot.turnRight();
