...
class Solution(object):
    def isBipartite(self, graph):
        ...
        while len(memo.keys()) > 0:
            k = list(memo.keys()).pop()
            tmp = 0
            sets[tmp].add(k)
            last_nbr = [k]
            curr_nbr = memo[k]
            del memo[k]
            # add all neighbor to sets1
            while len(curr_nbr) != 0:
                new_nbr = set()
                for idx in curr_nbr:
                    # check conflict
                    if idx in sets[tmp]:
                    ...
                    sets[1-tmp].add(idx)
                    # remove edges
                    for old_idx in last_nbr:
                        if old_idx in memo[idx]:
                            memo[idx].remove(old_idx)
                        if old_idx in memo and idx in memo[old_idx]:
                            memo[old_idx].remove(idx)
                    # memo[idx].remove(k)
                    for new_idx in memo[idx]:
                        new_nbr.add(new_idx)
                last_nbr, curr_nbr = curr_nbr, list(new_nbr)
                tmp = 1 - tmp
            remove_keys = []
            for k in memo.keys():
                if len(memo[k]) == 0:
