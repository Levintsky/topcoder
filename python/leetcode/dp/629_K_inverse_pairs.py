...
We define an inverse pair as following: For ith and jth 
element in the array, if i < j and a[i] > a[j] then it's an 
inverse pair; Otherwise, it's not.

Since the answer may very large, the answer should be modulo 
10**9 + 7.
...
Explanation: 
Only the array [1,2,3] which consists of numbers from 1 to 3 
has exactly 0 inverse pair.
...
dp[n][k] denotes the number of arrays that have k inverse pairs 
for array composed of 1 to n
we can establish the recursive relationship between dp[n][k] and dp[n-1][i]:

if we put n as the last number then all the k inverse pair 
should come from the first n-1 numbers
if we put n as the second last number then there's 1 inverse pair involves 
n so the rest k-1 comes from the first n-1 numbers
...
if we put n as the first number then there's n-1 inverse pairs involve n 
so the rest k-(n-1) comes from the first n-1 numbers

dp[n][k] = dp[n-1][k]+dp[n-1][k-1]+dp[n-1][k-2]+...+dp[n-1][k+1-n+1]
    +dp[n-1][k-n+1]
    ...
    def kInversePairs(self, n, K):
        ...
        for i in range(2, n+1):
            ...
            for k in range(1, K+1):
                ...
                # f(i,k) = f(i-1,k)+...+f(i-1, k-i+1)
                # f(i,k+1) = f(i-1,k+1)+f(i-1,k)+...+f(i-1,k-i+2)
                # we can deduce
                # f(i,k+1) = f(i,k) + f(i-1,k+1)-f(i-1,k-i+1)
                new_[k] = new_[k-1] + last_[k]
                if k-i >= 0: new_[k] -= last_[k-i]
            last_, new_ = new_, []
        return last_[-1]
...
if __name__ == '__main__':
    a = Solution()
    print(a.kInversePairs(8, 8))
    print(a.solution2(8, 8))
